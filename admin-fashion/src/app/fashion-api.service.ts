...
export class FashionAPIService {
  ...
  deleteFashion(fashionId:string):Observable<any>
  {
    ...
    return this._http.delete<any>("http://localhost:4000/fashions/"+fashionId,requestOptions).pipe(
    map(res=>JSON.parse(res) as Fashion),
    retry(3),
    catchError(this.handleError))
  }
  ...
  getFashionsByStyle(style: string): Observable<any> {
    ...
    return this._http.get<any>(`http://localhost:4000/fashions/style/${style}`, requestOptions).pipe(
      map(res => JSON.parse(res) as Array<Fashion>),
      retry(3),
      catchError(this.handleError)
    );
